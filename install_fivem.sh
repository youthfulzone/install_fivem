...
git clone https://github.com/youthfulzone/fivem_artifact.git ~/ftp/server/$1/

sudo tar xf ~/ftp/server/$name/fx.tar.xz -C ~/ftp/server/$1/
...
git clone https://github.com/youthfulzone/fivem_artifact.git ~/ftp/server/\$name/

sudo tar xf ~/ftp/server/\$name/fx.tar.xz -C ~/ftp/server/\$name/
...
# add system admins
add_ace group.admin command.quit deny # but don't allow quit
add_principal identifier.steam:110000112345678 group.admin # add the
 admin to the group
# remove the # to hide player endpoints in external log output
sv_endpointprivacy true
# server slots limit (must be between 1 and 31)
...
git clone https://github.com/youthfulzone/fivem_artifact.git ~/ftp/server/\$name/upgrade_files
